...
export default function MyProfile() {
  ...
  const loadData = async () => {
    ...
    // Fetch blog data
    let response = await fetch(`${process.env.BACKEND_URL}/api/myblogs`, {
      method: 'POST',
      headers: {
    ...
    setBlogCat(res[1]);
    // Fetch profile data
    response = await fetch(`${process.env.BACKEND_URL}/api/myprofiledata`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  const handleSave = async (e) => {
    ...
    await fetch(`${process.env.BACKEND_URL}/api/updateprofile`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  const handleShowFollowers = async() => {
    ...
    const followerEmails = profileData.follower || []; // Get the following email list
              if (followerEmails.length > 0) {
                const responseProfiles = await fetch(`${process.env.BACKEND_URL}/api/followerProfiles`, {
                  method: 'POST',
                  headers: {
                    'Content-Type': 'application/json',
                  },
  ...
  const handleShowFollowing = async() => {
    ...
    const followingEmails = profileData.following || []; // Get the following email list
              if (followingEmails.length > 0) {
                const responseProfiles = await fetch(`${process.env.BACKEND_URL}/api/followingProfiles`, {
                  method: 'POST',
                  headers: {
                    'Content-Type': 'application/json',
                  },
