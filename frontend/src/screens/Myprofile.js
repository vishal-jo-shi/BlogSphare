...
export default function MyProfile() {
  ...
  const loadData = async () => {
    ...
    // Fetch blog data
    let response = await fetch("http://localhost:4000/api/myblogs", {
      method: 'POST',
      headers: {
    ...
    setBlogCat(res[1]);
    // Fetch profile data
    response = await fetch("http://localhost:4000/api/myprofiledata", {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  const uploadFile = async (email, file) => {
    ...
    const response = await fetch("http://localhost:4000/api/upload", { method: 'POST', body: formData });
  ...
  const handleSave = async (e) => {
    ...
    await fetch('http://localhost:4000/api/updateprofile', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  const handleShowFollowers = async() => {
    ...
    const followerEmails = profileData.follower || []; // Get the following email list
              if (followerEmails.length > 0) {
                const responseProfiles = await fetch("http://localhost:4000/api/followerProfiles", {
                  method: 'POST',
                  headers: {
                    'Content-Type': 'application/json',
                  },
  ...
  const handleShowFollowing = async() => {
    ...
    const followingEmails = profileData.following || []; // Get the following email list
              if (followingEmails.length > 0) {
                const responseProfiles = await fetch("http://localhost:4000/api/followingProfiles", {
                  method: 'POST',
                  headers: {
                    'Content-Type': 'application/json',
                  },
